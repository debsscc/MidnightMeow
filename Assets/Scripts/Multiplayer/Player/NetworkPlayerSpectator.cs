...
public class NetworkPlayerSpectator : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        NetworkPlayerHealth.OnNetworkPlayerDowned += HandleAnyPlayerDowned;
        NetworkPlayerHealth.OnNetworkPlayerRevived += HandleAnyPlayerRevived;
    }
    ...
    private void RefreshAlivePlayers()
    {
        ...
        {
            if (player.CanFight && player.OwnerClientId != OwnerClientId)
                _alivePlayers.Add(player.transform);
        }
        _currentTargetIndex = 0;
    }

    private void HandleAnyPlayerDowned(ulong clientId)
    {
        if (_isSpectating) RefreshAlivePlayers();
    }

    private void HandleAnyPlayerRevived(ulong clientId)
    {
        if (_isSpectating) RefreshAlivePlayers();
    }
