...
public class CoraBarrierAbilityExecutor : MonoBehaviour, IAbilityExecutor
{
    ...
    public float Execute(AbilityTierData tierData, AbilityExecutionContext context)
    {
        ...
        if (spawner != null)
        {
            spawner.SpawnBarrier(barrierPrefab, context.PlacementPosition, tierData);
            return 0.2f;
        }

        var instance = Instantiate(barrierPrefab, context.PlacementPosition, Quaternion.identity);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (host != null)
            ...
            AbilityDebugGizmoUtility.DrawCircle(transform.position, definition.tier1.range,
                new Color(0.2f, 0.95f, 0.45f, 0.25f), new Color(0.5f, 1f, 0.65f, 0.9f));
    }
}
